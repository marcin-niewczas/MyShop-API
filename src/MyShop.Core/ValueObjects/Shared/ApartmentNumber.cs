...
public sealed record ApartmentNumber : IValidatableValueObject
{
    ...
    private static string GetErrorMessage()
        => $"The {nameof(ApartmentNumber)} must be between {MinLength} and {MaxLength} and not be whitespace.";
    ...
    public static void Validate(object? value, ICollection<ValidationMessage> validationMessages)
    {
        if (value is not null)
        {
            if (value is string stringValue)
            {
                ...
                {
                    validationMessages.Add(new(nameof(ApartmentNumber), [GetErrorMessage(stringValue)]));
                }
            }
            else
            {
                validationMessages.Add(
                    new(
                        ...
                        [$"The {nameof(ApartmentNumber)} must be a string.", GetErrorMessage()]
                        )
                    );
            }
        }
