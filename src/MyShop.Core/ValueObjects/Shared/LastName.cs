...
public sealed record LastName : IValidatableValueObject
{
    ...
    private static string GetErrorMessage()
        => $"The {nameof(LastName)} must be between {MinLength} and {MaxLength} and not be whitespace.";
    ...
    public static void Validate(object? value, ICollection<ValidationMessage> validationMessages)
    {
        if (value is string stringValue)
        {
            ...
            {
                validationMessages.Add(new(nameof(LastName), [GetErrorMessage(stringValue)]));
            }
        }
        else
        {
            validationMessages.Add(
                new(
                    nameof(LastName),
                    [$"The {nameof(LastName)} must be a string.", GetErrorMessage()]
                    )
                );
        }
    }
